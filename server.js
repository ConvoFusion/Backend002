...
io.on("connection", async (socket) => {
  ...
  if (user_id != null && Boolean(user_id)) {
    try {
      console.log("updating user online status with user id ",user_id);
      User.findByIdAndUpdate(user_id, {
        ...
        if (err){ 
            ...
            console.log("updated user status and socket id  "); 
        } })
  ...
  socket.on("start_conversation", async (data) => {
    ...
    if (existing_conversations.length === 0) {
      ...
      console.log("new chat created ",new_chat);
  ...
  socket.on("simple_request", ()=>{
    console.log('simple socket working');
  })
